...
void InitInputsandOutputs()
    {
        ...
        { // Setup each LED in order
            ...
            gpio_put(outs[i], 0);  // Initialize to low
        }
...
{    
    ...
    printf("Welcome to the SCR Timer!\n");
    sleep_ms(500);
    while(true)
    {
        ...
        printf("ADC Value: %d, Firing Delay: %d us\n", analogInput, firingDelay_us);
        sleep_ms(100);  // Small delay to avoid flooding the serial output
    }
}
