...
void textui_write_row0(const char *msg) {
    for (int c = 0; c < COLS_TEXT; ++c)
        textbuf[0][c] = ' ';
    int x = 0;
    for (const char *p = msg; *p && x < COLS_TEXT; ++p, ++x) 
        textbuf[0][x] = *p;
}
...
void textui_putc(char ch) {
    ...
    if (ch == '\b' || ch == 127) {
        if (cur_col > 0) {
            ...
            if (textbuf[cur_row][cur_col] == ' ' && cur_col > 0) 
                cur_col--;
            textbuf[cur_row][cur_col] = ' ';
        }
        return;
